...
class ResNetTransform:
  ...
  def __call__(self, tensor):
    with torch.no_grad():
      return self.net(tensor.to(device).unsqueeze(0)).squeeze(0).cpu()
...
data_transforms = {
    'train': transforms.Compose([
        ...
        SquarePad(DATA_SIZE),
        transforms.ColorJitter(0.2, 0.2, 0.2), # Randomly change the brightness, contrast and saturation of an image.
        transforms.RandomApply([MaskTransform()], p=0.5),
        transforms.ToTensor(),
